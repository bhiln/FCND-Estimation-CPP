...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // Quaternion for the euler angles for phi, theta, and psi.
  qt = qt.FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  
  // Quaternion that consists of the measurement of the angular rates from the IMU in the body frame.
  dq = dq.IntegrateBodyRate(gyro, dtIMU);
  
  // Define a predicted quaternion.
  Quaternion<float> qt_bar = dq * qt;
  
  // Extract predicted pitch, roll, and yaw from quaternion.
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  else if (diff < -F_PI) {zFromX(0) -= 2.f*F_PI;}
